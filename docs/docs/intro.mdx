...
| Area | Status |
|------|--------|
| [order-api](./services/order-api) — health + create order | **Built** |
| [catalog-api](./services/catalog-api) — health + product list (in-memory) | **Built** |
| PostgreSQL — orders schema | **Built** |
| Traefik — `/api/orders`, `/api/catalog` on `:8080` | **Built** |
| nginx | Planned |
| Redis cache, MinIO | Planned |
| Redpanda + event-processor | Planned |
| analytics-api + ClickHouse | Planned |
...
- **Service docs** — [catalog-api](./services/catalog-api), [order-api](./services/order-api)
...
| `order-api` | Go | Order creation, Postgres transactions *(event publishing planned)* |
