...
- Routed: `PathPrefix(/api/orders)` → order-api
- Direct access: order-api is still published on `:3002` for health checks and debugging
...
Catalog, analytics, TLS, and rate limiting are not wired yet.
...
| `/api/catalog/*` | nginx → catalog-api (2 replicas) | Planned |
