...
- Routed: `PathPrefix(/api/orders)` → order-api, `PathPrefix(/api/catalog)` → nginx (then 2× catalog-api), `PathPrefix(/api/analytics)` → analytics-api
...
Analytics routing is wired. TLS and rate limiting are not yet.
