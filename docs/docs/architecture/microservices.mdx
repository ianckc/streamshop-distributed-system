...
|---------|----------|-----------|------|----------|--------|
| `catalog-api` | Node.js 22 | Fastify | 3001 | 2 | Planned |
...
| Concern | Standard | order-api today |
|---------|----------|-----------------|
| Health | `GET /health` | Yes |
| Readiness | `GET /ready` | No |
| Tracing | OpenTelemetry → Jaeger | No |
| Metrics | Prometheus `/metrics` | No |
| Logging | Structured JSON | Yes (slog) |
| Config | Env via Compose | Yes |
| API spec | OpenAPI in `services/{name}/` | No |
