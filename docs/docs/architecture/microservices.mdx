...
|---------|----------|-----------|------|----------|--------|
| `catalog-api` | Node.js 22 + TypeScript | Fastify | 3001 | 2 | **Built** (health + Mongo list/detail + Redis cache) |
...
**Status:** **Implemented** (health + Mongo product list/detail + Redis cache-aside).

**Responsibility:** Product reads, Redis cache-aside; image upload URLs planned.

- Endpoints: `GET /health`, `GET /ready`, `GET /api/catalog/products`, `GET /api/catalog/products/:id`
- Writes/reads: MongoDB (`products`); Redis cache (list + detail)
- Planned: MinIO (images)
