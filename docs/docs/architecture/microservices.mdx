...
|---------|----------|-----------|------|----------|--------|
| `catalog-api` | Node.js 22 + TypeScript | Fastify | 3001 | 2 | **Built** (health + Mongo list/detail) |
...
**Status:** **Implemented** (health + Mongo product list/detail).

**Responsibility:** Product CRUD, image upload URLs, Redis cache-aside *(planned)*.

- Endpoints: `GET /health`, `GET /api/catalog/products`, `GET /api/catalog/products/:id`
- Writes/reads: MongoDB (`products`)
- Planned: MinIO (images), Redis cache-aside
