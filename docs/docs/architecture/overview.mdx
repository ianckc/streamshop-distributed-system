...
- **catalog-api** on `:3001` — `GET /health`, `GET /api/catalog/products`, `GET /api/catalog/products/:id` (also reachable directly)
...
**catalog-api today:** `GET /health`, `GET /api/catalog/products`, and `GET /api/catalog/products/:id` from MongoDB; Redis and MinIO are not wired yet.
