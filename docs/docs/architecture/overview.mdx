...
- **MinIO** — S3 API `:9000`, console `:9001`; buckets `product-images` and `exports` (empty); catalog-api does not upload yet
- No queue or other services yet
...
Today: `docker compose up` runs Traefik, nginx, postgres, mongo, redis, redisinsight, minio, order-api, catalog-api-1, and catalog-api-2.
...
**catalog-api today:** two replicas behind nginx; `GET /health`, `GET /ready` (Mongo ping), `GET /api/catalog/products`, and `GET /api/catalog/products/:id` with Redis cache-aside; MinIO buckets exist but upload is not wired yet.
...
| Object storage | MinIO | [Storage](./storage) | **Built** (Compose + buckets; upload planned) |
