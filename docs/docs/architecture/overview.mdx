...
- **MinIO** — S3 API `:9000`, console `:9001`; catalog-api does not upload yet
...
**catalog-api today:** two replicas behind nginx; `GET /health`, `GET /ready` (Mongo ping), `GET /api/catalog/products`, and `GET /api/catalog/products/:id` with Redis cache-aside; MinIO is in Compose but not wired yet.
...
| Object storage | MinIO | [Storage](./storage) | **Built** (Compose only; upload planned) |
