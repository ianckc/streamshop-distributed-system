...
flowchart LR
  ...
  Traefik --> CatalogAPI
  Traefik --> OrderAPI
  CatalogAPI --> Mongo[(MongoDB_8)]
  CatalogAPI --> Redis[(Redis_8)]
  OrderAPI --> Postgres[(PostgreSQL_18)]
```

- **Traefik** on `:8080` — `/api/orders` → order-api, `/api/catalog` → catalog-api; dashboard on `:8081`
- **catalog-api** on `:3001` — `GET /health`, `GET /ready`, `GET /api/catalog/products`, `GET /api/catalog/products/:id` (also reachable directly)
...
Today: `docker compose up` runs Traefik, postgres, mongo, redis, redisinsight, order-api, and catalog-api.
...
**catalog-api today:** `GET /health`, `GET /ready` (Mongo ping), `GET /api/catalog/products`, and `GET /api/catalog/products/:id` with Redis cache-aside; MinIO is not wired yet.
