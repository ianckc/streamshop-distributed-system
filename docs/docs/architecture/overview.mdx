...
Today: `docker compose up` runs Traefik, nginx, postgres, mongo, redis, redisinsight, minio, redpanda, redpanda-console, clickhouse, event-processor, order-api, catalog-api-1, and catalog-api-2.
...
4. **Async processing** — Rust consumer → ClickHouse + `processed` *(analytics API not built)*
