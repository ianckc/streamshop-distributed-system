...
**order-api today:** writes to PostgreSQL then publishes `order.created` to `orders.events` (fail-open); optional `Idempotency-Key` via Redis; exposes `GET /health`, `GET /ready` (Postgres ping), and `POST /api/orders`.
...
3. **Place order** Go service writes to Postgres, publishes `order.created` *(built)*
