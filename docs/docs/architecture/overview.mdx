...
- **Traefik** on `:8080` — path prefix `/api/orders` → order-api; dashboard on `:8081`
- **order-api** on `:3002` — `GET /health`, `POST /api/orders` (also reachable directly)
- **PostgreSQL** — `orders` and `order_items` tables
- No catalog, queue, or other services yet

Details: [order-api](../services/order-api), [Quick Start](../getting-started/quick-start).
...
Today: `docker compose up` runs Traefik, postgres, and order-api.
...
|-----------|------------|-----|--------|
| API Gateway | Traefik v3 | [API Gateway](./api-gateway) | **Built** (`/api/orders` only) |
