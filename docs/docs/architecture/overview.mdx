...
- **Redis** — running in Compose; catalog-api does not cache yet
...
**catalog-api today:** `GET /health`, `GET /ready` (Mongo ping), `GET /api/catalog/products`, and `GET /api/catalog/products/:id`; Redis is in Compose but not used; MinIO is not wired yet.
...
| Cache | Redis 8 | [Caching](./caching) | **Built** (Compose only; cache-aside planned) |
