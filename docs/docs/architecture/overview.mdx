...
- **order-api** on `:3002` — `GET /health`, `POST /api/orders`
- **PostgreSQL** — `orders` and `order_items` tables
- No gateway, queue, or other services yet
...
Today: `docker compose up` runs postgres + order-api only.
...
|-----------|------------|-----|--------|
| API Gateway | Traefik v3 | [API Gateway](./api-gateway) | Planned |
