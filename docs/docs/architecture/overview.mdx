...
- **catalog-api** on `:3001` — `GET /health`, `GET /api/catalog/products` (also reachable directly)
...
**catalog-api today:** `GET /health` and `GET /api/catalog/products` from MongoDB; Redis and MinIO are not wired yet.
