...
Today: `docker compose up` runs Traefik, nginx, postgres, mongo, redis, redisinsight, order-api, catalog-api-1, and catalog-api-2.
...
**catalog-api today:** two replicas behind nginx; `GET /health`, `GET /ready` (Mongo ping), `GET /api/catalog/products`, and `GET /api/catalog/products/:id` with Redis cache-aside; MinIO is not wired yet.
