...
Today: `docker compose up` runs Traefik, postgres, mongo, order-api, and catalog-api.
...
**catalog-api today:** `GET /health`, `GET /ready` (Mongo ping), `GET /api/catalog/products`, and `GET /api/catalog/products/:id`; Redis and MinIO are not wired yet.
