...
| Message Queue | Redpanda | [Messaging](./messaging) | **Built** (`order.created` produce) |
...
3. **Place order** — Go service writes to Postgres, publishes `order.created` *(consume/analytics not built)*
