...
Redis is in Docker Compose (`localhost:6379`, no password) and is **shared** across the two catalog-api replicas behind nginx. catalog-api uses **cache-aside** on product list and detail: Redis first, Mongo on miss, then `SET` with TTL **60 seconds**. Redis stores Mongo fields only; presigned `image_url` values are attached after the cache so they do not sit in Redis until they expire. Redis down does not fail the request, Mongo still serves (`x-cache: miss`). `/ready` pings Mongo only.
